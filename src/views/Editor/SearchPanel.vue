<template>
  ...
  >
    ...
    <div class="content" :class="type" @mousedown.stop>
      <Input class="input" v-model:value="searchWord" placeholder="Find text" @enter="searchNext()" ref="searchInpRef">
        <template #suffix>
      ...
      <Input class="input" v-model:value="replaceWord" placeholder="Replace" @enter="replace()" v-if="type === 'replace'"></Input>
      <div class="footer" v-if="type === 'replace'">
        <Button :disabled="!searchWord" style="margin-left: 5px;" @click="replace()">Replace</Button>
        <Button :disabled="!searchWord" type="primary" style="margin-left: 5px;" @click="replaceAll()">AllReplace</Button>
      </div>
    </div>
  </MoveablePanel>
</template>
