  ...
  >
    <div class="container">
      ...
      <div class="row" v-else-if="handleElement && handleElement.type === 'image'">
        <div style="width: 40%;">Image type：</div>
        <Select
          style="width: 60%;"
          :value="imageType"
          @update:value="value => updateElement(value as Image type | '')"
          :options="imageTypeOptions"
        />
      </div>
      <div class="placeholder" v-else>Select an image, text, or text shape to mark its type</div>
    </div>
  </MoveablePanel>
</template>
...
import type { Image type, SlideType, TextType } from '@/types/slides'
...
const textTypeOptions = ref<{ label: string; value: TextType | '' }[]>([
  ...
  { label: '', value: 'content' },
  { label: 'Columns', value: 'item' },
  { label: 'Columns', value: 'itemTitle' },
  { label: '', value: 'notes' },
  { label: '', value: 'header' },
  { label: '', value: 'footer' },
  { label: 'Numbering', value: 'partNumber' },
  { label: 'Numbering', value: 'itemNumber' },
])

const imageTypeOptions = ref<{ label: string; value: Image type | '' }[]>([
  { label: 'Type', value: '' },
  { label: '', value: 'pageFigure' },
  { label: '', value: 'itemFigure' },
...
const updateElement = (type: TextType | Image type | '') => {
  if (!handleElement.value) return
  if (handleElement.value.type === 'image') {
    if (type) {
      slidesStore.updateElement({ id: handleElementId.value, props: { imageType: type as Image type } })
    }
    else {
