  ...
  <div class="export-img-dialog">
    ...
    <div class="configs">
      ...
      <div class="row" v-if="rangeType === 'custom'">
        <div class="title" :data-range="`（${range[0]} ~ ${range[1]}）`">Custom：</div>
        <Slider
          class="config-item"
          range
          :min="1"
      ...
      <div class="row">
        ...
        <div class="config-item">
          <Switch v-model:value="ignoreWebfont" v-tooltip="'Font，SlideMediumFont，Style，SelectClose「Font」，。'" />
        </div>
      </div>
    </div>

    <div class="btns">
      <Button class="btn export" type="primary" @click="expImage()"><i-icon-park-outline:download /> Image</Button>
      <Button class="btn close" @click="emit('close')">Close</Button>
    </div>

    <FullscreenSpin :loading="exporting" tip="..." />
  </div>
</template>
