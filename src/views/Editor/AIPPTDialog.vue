<template>
  <div class="aippt-dialog">
    <div class="header">
      ...
      <span class="subtite" v-if="step === 'template'">Choose a template below to generate a deck, or <span class="local" v-tooltip="'Upload a .pptist template file'" @click="uploadLocalTemplate()">use a local template</span></span>
      <span class="subtite" v-else-if="step === 'outline'">ConfirmLarge（，Add/DeleteLarge），Select</span>
      <span class="subtite" v-else>PPT，，Rows、、、</span>
    </div>
    ...
    <template v-if="step === 'setup'">
      <Input class="input" 
        ...
        :maxlength="50" 
        placeholder="PPT，：Large" 
        @enter="createOutline()"
      >
        <template #suffix>
          <span class="count">{{ keyword.length }} / 50</span>
          <div class="submit" type="primary" @click="createOutline()"><i-icon-park-outline:send class="icon" /> Generate with AI</div>
        </template>
      </Input>
        ...
        <div class="config-item">
          ...
          <Select 
            ...
            :options="[
              { label: 'None', value: '' },
              { label: '', value: 'test' },
              { label: 'AI', value: 'ai-search', disabled: true },
              { label: 'AI', value: 'ai-create', disabled: true },
            ]"
          />
        </div>
      </div>
      <div class="configs" v-if="!isEmptySlide">
        <div class="config-item">
          <Checkbox v-model:value="overwrite">Slide</Checkbox>
        </div>
      </div>
    </template>
    <div class="preview" v-if="step === 'outline'">
      ...
      <div class="btns" v-if="!outlineCreating">
        <Button class="btn" type="primary" @click="step = 'template'">Select</Button>
        <Button class="btn" @click="outline = ''; step = 'setup'">BackGenerate</Button>
      </div>
    </div>
    <div class="select-template" v-if="step === 'template'">
      ...
      <div class="btns">
        ...
        <Button class="btn" @click="step = 'outline'">BackLarge</Button>
      </div>
    </div>

    <FullscreenSpin :loading="loading" tip="AIGenerateMedium， ..." />
  </div>
</template>
...
const createOutline = async () => {
  if (!keyword.value) return message.error('Enter a presentation topic first')
  ...
  if (typeof stream === 'object' && stream.state === -1) {
    loading.value = false
    return message.error('ModelAPI，ChangeModel')
  }
...
const createPPT = async (template?: { slides: Slide[], theme: SlideTheme }) => {
  ...
  message.loading('GenerateMedium， ...', { duration: 0 })
