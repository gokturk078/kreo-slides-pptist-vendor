  ...
  <div class="rich-text-base">
    ...
    <ButtonGroup class="row" passive>
      <Popover trigger="click" v-model:value="AIPopoverVisible" style="width: 25%;">
        <template #content>
          <PopoverMenuItem center @click="execAI('Polish rewrite')">Polish</PopoverMenuItem>
      ...
      <CheckboxButton
        ...
        v-tooltip="'Format painter (double-click to keep active)'"
...
const updateLink = (link?: string) => {
  ...
  if (!link || !linkRegExp.test(link)) return message.error('Enter a valid web link.')
...
const execAI = async (command: string) => {
  ...
  if (!content) return message.error('Rows')
