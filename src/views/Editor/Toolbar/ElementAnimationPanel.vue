<template>
  <div class="element-animation-panel">
    ...
    <div class="tip" v-else><i-icon-park-outline:click style="margin-right: 5px;" /> Select a canvas element to add animation</div>
    ...
    <template v-if="animationSequence.length >= 2">
      ...
      <Button @click="runAllAnimation()">
        <i-icon-park-outline:pause v-if="animateIn" /><i-icon-park-outline:play-one v-else /> {{ animateIn ? 'Preview' : 'Preview all'}}
      </Button>
    </template>
  </div>
</template>
