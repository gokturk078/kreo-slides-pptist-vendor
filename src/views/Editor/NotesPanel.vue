      ...
      <div class="notes" ref="notesRef">
        <div class="note" :class="{ 'active': activeNoteId === note.id }" v-for="note in notes" :key="note.id" @click="handleClickNote(note)">
          ...
          <div class="note-reply" v-if="replyNoteId === note.id">
            <TextArea :padding="6" v-model:value="replyContent" placeholder="Write a reply" :rows="1" @enter.prevent="createNoteReply()" />
        ...
        <div class="empty" v-if="!notes.length">No comments on this slide</div>
      </div>
      <div class="send">
        <TextArea 
          ref="textAreaRef"
          ...
          :padding="6"
          :placeholder="`Add a comment for ${handleElementId ? 'the selected element' : 'this slide'}`"
          :rows="2"
          @focus="replyNoteId = ''; activeNoteId = ''"
          @enter.prevent="createNote()"
        />
        <div class="footer">
          <i-icon-park-outline:delete class="btn icon" v-tooltip="'Clear slide comments'" style="flex: 1" @click="clear()" />
          <Button type="primary" class="btn" style="flex: 12" @click="createNote()"><i-icon-park-outline:plus /> Add comment</Button>
        </div>
      </div>
    </div>
  </MoveablePanel>
