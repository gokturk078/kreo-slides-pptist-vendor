...
export default (elementList: Ref<PPTElement[]>) => {
  ...
  const getPagePoint = (e: MouseEvent | TouchEvent) => {
    if (e instanceof MouseEvent) return { x: e.pageX, y: e.pageY }
    const touch = e.changedTouches[0]
    return touch ? { x: touch.pageX, y: touch.pageY } : null
  }

  const dragLineElement = (e: MouseEvent | TouchEvent, element: PPTLineElement, command: OperateLineHandlers) => {
    const startPoint = getPagePoint(e)
    ...
    const startPageX = startPoint.x
    const startPageY = startPoint.y
    ...
    const handleMove = (e: MouseEvent | TouchEvent) => {
      ...
      const currentPageX = point.x
      const currentPageY = point.y
    ...
    const handleEnd = (e: MouseEvent | TouchEvent) => {
      ...
      if (startPageX === point.x && startPageY === point.y) return
